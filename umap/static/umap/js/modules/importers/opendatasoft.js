...
const TEMPLATE = `
  <div>
    <h3>OpenDataSoft</h3>
    <p>${translate('Import data from an OpenDataSoft instance')}.</p>
    <div class="formbox">
      <select name="instance" data-ref="instances">
        <option disabled selected value="">${translate('Choose an instance')}</option>
      </select>
...
export class Importer {
  constructor(umap, options = {}) {
    this.umap = umap
    this.name = options.name || 'OpenDataSoft'
    this.id = 'opendatasoft'
    this.instances = options.choices || INSTANCES
  }

  async open(importer) {
    let fields_map = {}
    const [container, { instances, datasets, geofield }] =
      Utils.loadTemplateWithRefs(TEMPLATE)
    instances.addEventListener('change', async (event) => {
