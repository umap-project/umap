...
def test_can_change_name(live_server, openmap, page, datalayer):
    ...
    expect(page.locator("body")).to_contain_class("umap-is-dirty")
...
def test_can_create_new_datalayer(live_server, openmap, page, datalayer):
    ...
    page.wait_for_timeout(300)  # Wait for the input debounce.
    with page.expect_response(re.compile(".*/datalayer/create/.*")):
        page.get_by_role("button", name="Save").click()
    assert DataLayer.objects.count() == 2
    saved = DataLayer.objects.last()
...
def test_parent_layer_should_always_be_saved_before_children(
    ...
    [child_id, parent_id] = page.evaluate("() => U.MAP.layers.tree.map((d) => d.id)")
