...
def bootstrap(map, live_server):
    ...
    # Center on a vertex of the line so clicking the map will click the line
    # (we cannot target the line easily otherwise, given the map is a whole canvas).
    map.settings["geometry"] = {
        "type": "Point",
        "coordinates": [11.25, 53.585984],
    }
    map.save()
...
def test_should_open_popup_on_click(
    live_server, map, page, bootstrap, wait_for_loaded, assert_screenshot
):
    map_canvas = page.locator("#map")
    page.goto(f"{live_server.url}{map.get_absolute_url()}")
    wait_for_loaded(page)
    clip = {"x": 585, "y": 365, "width": 70, "height": 50}
    assert_screenshot(page, suffix="default", clip=clip)
    # Will click on the center, so on the line.
    map_canvas.click()
